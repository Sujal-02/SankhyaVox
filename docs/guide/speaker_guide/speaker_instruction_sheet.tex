...
\vspace{4pt}
\textbf{Tip:} Before each repetition, silently count the syllables of the word,
then speak it --- this prevents syllable-dropping mid-session.
\end{infobox}
...
\vspace{0.3cm}
\begin{warnbox}
\textbf{Critical: Do NOT use Hindi equivalents.}
Say the full Sanskrit word. Count the syllables before speaking:
\begin{itemize}[leftmargin=1.2em, itemsep=2pt]
  \item \textbf{dvi} = 1 syllable (DWEE) --- NOT ``dva'', NOT ``do''
  \item \textbf{catur} = 2 syllables (cha-TUR) --- NOT ``char'', NOT ``chaar''
  \item \textbf{pa\~{n}ca} = 2 syllables (PUN-cha) --- NOT ``panch''
  \item \textbf{da\'{s}a} = 2 syllables (da-SHA) --- NOT ``das''
  \item \textbf{vi\d{m}\'{s}ati} = 3 syllables (vim-SHA-ti) --- NOT ``bees''
  \item \textbf{a\d{s}\d{t}a} = 2 syllables (USH-ta) --- NOT ``aath''
\end{itemize}
If you are unsure, listen to the reference audio before recording.
\end{warnbox}
...
\textbf{Pronunciation} & \textbf{Say this} & \textbf{NOT this} \\
\midrule
 1  & \'{s}\={u}nya       & 0   & SHOON-ya   & \texttt{S\_\_\_shunya\_raw.wav}  & --- \\
 2  & eka                  & 1   & AY-ka      & \texttt{S\_\_\_eka\_raw.wav}     & --- \\
 3  & dvi                  & 2   & \textbf{DWEE} (1 syl.)    & \texttt{S\_\_\_dvi\_raw.wav}     & ``dva'', ``do'' \\
 4  & tri                  & 3   & TREE (1 syl.)             & \texttt{S\_\_\_tri\_raw.wav}     & --- \\
 5  & catur                & 4   & \textbf{cha-TUR} (2 syl.) & \texttt{S\_\_\_catur\_raw.wav}   & ``char'', ``chaar'' \\
 6  & pa\~{n}ca            & 5   & \textbf{PUN-cha} (2 syl.) & \texttt{S\_\_\_pancha\_raw.wav}  & ``panch'' \\
 7  & \d{s}a\d{t}          & 6   & \textbf{SHUT} (1 syl.)    & \texttt{S\_\_\_shat\_raw.wav}    & ``chhe'', ``saat'' \\
 8  & sapta                & 7   & SUP-ta (2 syl.)           & \texttt{S\_\_\_sapta\_raw.wav}   & --- \\
 9  & a\d{s}\d{t}a         & 8   & \textbf{USH-ta} (2 syl.)  & \texttt{S\_\_\_ashta\_raw.wav}   & ``aath'', ``attha'' \\
10  & nava                 & 9   & NUH-va (2 syl.)           & \texttt{S\_\_\_nava\_raw.wav}    & --- \\
11  & da\'{s}a             & 10  & \textbf{da-SHA} (2 syl.)  & \texttt{S\_\_\_dasha\_raw.wav}   & ``das'', ``duss'' \\
12  & vi\d{m}\'{s}ati      & 20  & \textbf{vim-SHA-ti} (3!)  & \texttt{S\_\_\_vimsati\_raw.wav} & ``bees'', ``vimsht'' \\
13  & \'{s}ata             & 100 & SHA-ta (2 syl.)           & \texttt{S\_\_\_shata\_raw.wav}   & --- \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}[leftmargin=1.5em, itemsep=4pt]
  \item \textbf{Before you start:} Listen to the reference recordings sent by
        the project coordinator. Try to match the style, speed, and syllable
        count --- not the exact accent.
